...
class ERC20 {
  ...
  async totalSupply() {
    ...
    try {
      const response = await axios.get(`${this.restURL}dataRetrieval/property/${this.token.propertyid}`);
  ...
  async balanceOf(owner) {
    ...
    try {
      const response = await axios.get(`${this.restURL}dataRetrieval/balancesForAddress/${owner}`);
      let token;
  ...
  async transfer(to, value, cb) {
    ...
    try {
      ...
      const masterHDNode = BITBOX.HDNode.fromSeed(rootSeed, BITBOX.Address.detectAddressNetwork(to) === "mainnet" ? "bitcoincash" : "testnet");
      ...
      const changeHex = await this.RawTransactions.change(ref, // Raw transaction we're working with.
      [utxo], // Previous utxo
